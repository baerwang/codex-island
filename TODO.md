...
- [x] 所有 provider CLI 状态操作都必须显式设置该 provider 的 `HTTP_PROXY` 与
  `HTTPS_PROXY`；配置为空时不得启动该 provider 的 CLI 状态操作。除这两个代理变量外，
  不额外干预或清理子进程继承的环境变量；Claude 额外设置用户指定的本地 `NO_PROXY`。
...
- [x] 同时存在多个 Codex 配置项时，主视图不得自动选取“默认账号”；显示多个配置项状态，
  展开页必须能看到每个配置项的独立状态。
...
- [x] 多 Codex 配置项在灵动岛不显示账号汇总、轮播或隐式默认项；展开页按配置分别展示。
