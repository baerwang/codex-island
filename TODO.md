...
- [x] 用“CLI 状态”“本地日志估算”替换所有暗示 OAuth/API 直连的 UI、帮助文案、错误文案和
  隐私声明。
- [x] Codex 本地消耗页默认汇总全部启用配置，可按用户填写的配置名筛选；切换菜单期间保持
  灵动岛展开，第四页固定高度，并明确显示“扫描中/该配置暂无本地记录”，不得缩小或黑屏。
- [x] 接近限额告警跟随 Claude/Codex 各自选择的 5 小时或周额度窗口，缺少该窗口时才回退
  到 CLI 实际报告的另一窗口；阈值统一按已用百分比判断。
...
- [x] 为多 `CODEX_HOME` 配置、项目路径归属、同名路径、子代理、未归属事件、日/月边界和
  多账号不合并建立单元测试。
- [x] 每次 Codex 轮询只使用一个 PTY 会话；每 3 秒自适应发送 `/status`，识别成功立即停止，
  最多三次。配置 Codex 时仅定向刷新 Codex，不得顺带重复启动 Claude 会话。
